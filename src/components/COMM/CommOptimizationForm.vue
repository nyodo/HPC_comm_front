  ...
  <div class="comm-optimization-content">
    <el-card>
      ...
      >
        <el-row :gutter="20">
          <el-col :span="24">
            <el-form-item label="输入来源" prop="input_mode">
              <el-radio-group v-model="commForm.input_mode">
                <el-radio-button label="collection">历史采集任务（归档）</el-radio-button>
                <el-radio-button label="upload">手动上传CSV</el-radio-button>
              </el-radio-group>
            </el-form-item>
          </el-col>

          <el-col v-if="commForm.input_mode === 'collection'" :span="24">
            <el-form-item label="采集任务" prop="collection_id">
              <el-select
                v-model="commForm.collection_id"
                filterable
                remote
                clearable
                placeholder="选择已归档的采集任务"
                :remote-method="remoteSearchCollections"
              ...
              >
                <el-option
                  ...
                  :label="`${item.name || item.collection_id}  (max=${formatMaxTime(item.max_of_rank_max_comm_time_s)}s)`"
          ...
          <el-col v-if="commForm.input_mode === 'collection'" :span="24">
            <el-form-item label="摘要">
              <el-tag v-if="selectedCollectionMaxS != null" type="success">max_of_rank_max_comm_time={{ formatMaxTime(selectedCollectionMaxS) }} s</el-tag>
          ...
          <el-col :span="24">
            <el-form-item label="原始CSV文件（多选）" prop="raw_files">
              <el-upload
                ...
                multiple
                drag
                :disabled="commForm.input_mode !== 'upload'"
              >
...
const selectedCollectionMaxS = ref(null);

function formatMaxTime(v) {
  if (v == null) return "-";
  const n = Number(v);
...
const commRules = {
  ...
  input_mode: [{ required: true, message: "请选择输入来源", trigger: "change" }],
  collection_id: [
    {
      validator: (_, value, cb) => {
        if (commForm.input_mode === "collection" && !value) return cb(new Error("请选择采集任务"));
        cb();
      },
      trigger: "change",
    },
  ...
  raw_files: [
    {
      validator: (_, value, cb) => {
        if (commForm.input_mode === "upload" && (!Array.isArray(value) || value.length < 1)) {
          return cb(new Error("请至少上传 1 个 CSV 文件"));
        }
        cb();
      },
      trigger: "change",
...
const remoteSearchCollections = async (q) => {
  collectionsLoading.value = true;
  try {
    const resp = await axios.get("/api/mt3000/comm/collections", {
      params: { page: 1, page_size: 20, status: "archived", query: q || undefined },
    });
    if (resp.data?.status !== "success") throw new Error(resp.data?.message || "查询失败");
    collectionOptions.value = resp.data.data?.items || [];
  } catch {
...
const onCollectionChange = async (collectionId) => {
  selectedCollectionMaxS.value = null;
  if (!collectionId) return;
  try {
    const resp = await axios.get(`/api/mt3000/comm/collections/${collectionId}`);
    if (resp.data?.status !== "success") throw new Error(resp.data?.message || "查询失败");
    selectedCollectionMaxS.value = resp.data.data?.max_of_rank_max_comm_time_s ?? null;
  } catch {
    selectedCollectionMaxS.value = null;
  }
};
...
const normalizeCommPayload = () => {
  ...
  return {
    name: commForm.name,
    input_mode: commForm.input_mode,
    collection_id: commForm.collection_id || undefined,
...
const startTuning = async () => {
  try {
    ...
    const endpoint =
      commForm.input_mode === "collection"
        ? "/api/mt3000/optimization/communication/from-collection"
        : "/api/comm-collection";
